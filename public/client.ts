    ...
    if (command === "toggle-camera") {
      if (!rtc.hasLocalCameraStream()) {
        ...
        if (!state.cameraPermissionApproved) {
          ui.showToast(
            "Host requested camera. Please allow Camera + Voice once from request button.",
            "error",
          );
          return;
        }
...
function refreshLocalMediaToggleState(): void {
...
function sendHostRemoteMediaCommand(command: RemoteMediaCommand): boolean {
  if (state.role !== "host") {
    return false;
  }
