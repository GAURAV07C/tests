...
function isIdentifyPayload(payload: unknown): payload is IdentifyPayload {
  return isRecord(payload) && isUserId(payload.userId);
}

function isRoomJoinPayload(payload: unknown): payload is RoomJoinPayload {
  return isRecord(payload) && isRoomId(payload.roomId);
}

function isRejoinRoomPayload(payload: unknown): payload is RejoinRoomPayload {
  return (
    isRecord(payload) && isRoomId(payload.roomId) && isRole(payload.role)
  );
}

function isControlStatusPayload(payload: unknown): payload is ControlStatusPayload {
  return (
    isRecord(payload) && isRoomId(payload.roomId) && isBoolean(payload.active)
  );
}

function isCameraRequestPayload(payload: unknown): payload is CameraRequestPayload {
  return isRecord(payload) && isRoomId(payload.roomId);
}

function isCameraPermissionPayload(
  payload: unknown,
): payload is CameraPermissionPayload {
  return (
    isRecord(payload) &&
...
function isCameraStatePayload(payload: unknown): payload is CameraStatePayload {
  return (
    isRecord(payload) && isRoomId(payload.roomId) && isBoolean(payload.active)
  );
}

function isMediaKindPayload(payload: unknown): payload is MediaKindPayload {
  return (
    isRecord(payload) &&
    isRoomId(payload.roomId) &&
...
function isIceCandidatePayload(payload: unknown): payload is IceCandidatePayload {
  if (!isRecord(payload)) {
    return false;
  }
...
function isWebRtcSignal(payload: unknown): payload is WebRtcSignal {
...
function isWebRtcSignalPayload(payload: unknown): payload is WebRtcSignalPayload {
  return (
    isRecord(payload) && isRoomId(payload.roomId) && isWebRtcSignal(payload.signal)
  );
}
...
function getUserId(socket: Socket): string | null {
  const raw = (socket.data as Record<string, unknown>).userId;
  return typeof raw === "string" ? raw : null;
}

function isCurrentSocketForRole(room: Room, role: Role, socketId: string): boolean {
  return role === "host"
    ? room.hostSocketId === socketId
    : room.clientSocketId === socketId;
}

function roleOfUserInRoom(room: Room, userId: string): Role | null {
...
function roomPeerSocketId(room: Room, role: Role): string | undefined {
...
function normalizeRoomState(room: Room): void {
  let changed = false;
...
function socketUserIdBySocketId(io: Server, socketId: string): string | null {
...
function restoreSession(io: Server, socket: Socket, room: Room, role: Role): void {
...
function ensureIdentified(socket: Socket): string | null {
...
export function registerSocketHandlers(io: Server): void {
  io.on("connection", (socket) => {
    ...
    socket.on("control:status", (payload: unknown) => {
    ...
    socket.on("camera:state", (payload: unknown) => {
